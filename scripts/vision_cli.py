...
FORMAT_PRESETS = {
    ...
    "wechat_chat": """请识别微信聊天截图内容，并输出 JSON 格式。

识别要点：
1. 判断截图来源：
   - PC端微信：窗口通常较宽，消息气泡横向分布，顶部可能显示窗口控制按钮
   - 手机端微信：窗口较窄，消息气泡纵向排列，顶部可能显示状态栏、时间和电量图标

2. 消息气泡颜色识别：
   - 绿色气泡：当前登录用户（自己）发送的消息，通常在右侧
   - 白色/灰色气泡：对方发送的消息，通常在左侧
   - 注意：不同微信版本和主题可能会影响颜色深浅，请根据气泡位置和颜色综合判断

3. 输出 JSON 格式要求：
{
  "platform": "pc" 或 "mobile",
  "chat_title": "聊天窗口顶部显示的聊天对象名称",
  "messages": [
    {
      "sender": "self" 或 "other",
      "content": "消息文字内容",
      "time": "消息时间（如果有显示）",
      "type": "text" 或 "image" 或 "voice" 或 "video" 或 "link" 或 "other"
    }
  ],
  "message_count": 消息总数,
  "self_message_count": 自己发送的消息数,
  "other_message_count": 对方发送的消息数
}

注意事项：
- 如果消息包含图片、语音、视频等多媒体内容，请在 content 中描述，type 标记为对应类型
- 如果无法识别某些文字，用 [无法识别] 标记
- 按照从上到下的顺序排列消息
- 如果有多张图片拼接的长截图，请分段识别并合并输出"""
}
