...
*   **场景化预置 (Presets)**：内置 `ppt`, `business_flat`, `tech_isometric`, `hand_drawn` 等生成风格，以及 `invoice`, `contract`, `form`, `slide`, `whiteboard`, `table`, `json`, `wechat_chat` 等识别输出格式，让 Agent 调用更智能。
*   **高可用性设计**：支持备用模型配置（FALLBACK_MODEL），主模型失败时自动切换，提高任务成功率。
*   **批处理与稳定性增强**：支持一次识别多张图片，内置失败重试、可选降级模型，面对网络抖动和接口波动更稳定。
*   **可观测任务元数据**：任务状态中包含 `started_at / ended_at / duration_ms / api_attempts / upload_mode` 等字段，便于排障和监控。
