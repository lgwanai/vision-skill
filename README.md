...
*   **异步任务架构**：采用异步任务队列设计，大任务不阻塞 Agent 对话流，随时轮询结果。
...
3.  **流程自动化**：从本地图片 -> 上传云端 -> 识别/生成 -> 结果回传，全流程自动化，Agent 只需调用一条指令。

## 🚀 使用场景

*   **UI/UX 设计辅助**：上传草图，生成高保真渲染图；或者根据现有 UI 生成变体。
*   **文档数字化**：批量识别发票、海报、文档中的文字并结构化输出。
*   **内容创作**：为文章自动配图，或制作连贯的故事绘本。
...
```bash
# 克隆仓库
git clone https://github.com/your-repo/vision-skill.git
cd vision-skill
...
# 识别网络图片
python3 scripts/vision_cli.py recognize https://example.com/image.jpg
```
...
# 连续生成 (生成4张连贯图)
python3 scripts/vision_cli.py generate "四季变化的树" --seq 4
```
...
**Q: 为什么识别结果有延迟？**
A: 视觉大模型处理需要时间，尤其是高清图片分析或批量生成任务。通常识别在 3-5秒，生成在 10-30秒左右。
